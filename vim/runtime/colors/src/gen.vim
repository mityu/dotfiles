...
var Acknowledgements: list<list<string>>
...
export def Acknowledgement(lines: list<string>)
  add(Acknowledgements, lines)
enddef

export def Generate()
  # TODO: Check ColorsInfo has required information.

  const header =<< trim eval END
  " vim: set expandtab smarttab shiftwidth=2
  " Generated by gen.vim on {strftime('%d %b %Y')}

  END

  const initializer =<< trim eval END
  set background={ColorsInfo.background}
  highlight clear
  if exists('g:syntax_on')
    syntax reset
  endif
  let g:colors_name = {fnamemodify(ColorsInfo.name, ':r')->string()}

  END

  var lines: list<string>
  lines->extend(header)
  lines->extend(GenerateAcknowledgementsSection())
...
def GenerateAcknowledgementsSection(): list<string>
  var script = []
  for block in Acknowledgements
    script->extend(block->flattennew())
    script->add('')
  endfor
  return script
