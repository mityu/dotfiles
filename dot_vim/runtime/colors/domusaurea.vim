...
function! s:hi(group,fg,bg,attr) "{{{
    ...
    let [type, idx] = has('gui_running') ? ['gui', 1] : ['cterm', 0]
    let fg = has_fg ? printf('%sfg=%s', type, s:palette[a:fg][idx]) : ''
    let bg = has_bg ? printf('%sbg=%s', type, s:palette[a:bg][idx]) : ''
